...
if __name__ == "__main__":
    ...
    print(f"✅ Adversarial dataset generation completed successfully. Total rounds: {len(history)}")�「{record['player_attack']}」。請寫一句極富創意且辛辣的話嗆回去，不超過25字，不要有多餘說明。\nAssistant: {record['player_response']}"
                })
                temp_referee.append({
                    "text": f"User: 你是一個毒舌裁判。請評估以下攻擊，並以 JSON 格式輸出評估結果，格式為：\n{{\"damage\": 10到50之間的整數, \"referee_comment\": \"20字內的毒舌短評\"}}\n\n玩家攻擊：「{record['player_response']}」\nAssistant: {{\"damage\": {record['damage']}, \"referee_comment\": \"{record['comment']}\"}}"
                })
            with open("player_train.json", "w", encoding="utf-8") as f:
                json.dump(temp_player, f, ensure_ascii=False, indent=2)
            with open("referee_train.json", "w", encoding="utf-8") as f:
